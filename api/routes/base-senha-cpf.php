...
require_once __DIR__ . '/../src/controllers/BaseSenhaCpfController.php';
...
try {
    ...
    $controller = new BaseSenhaCpfController($db);
    $method = $_SERVER['REQUEST_METHOD'];
    $path = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);

    // Remover prefixo da API se presente
    $basePaths = ['/api/base-senha-cpf', '/base-senha-cpf'];
    ...
    switch ($method) {
        ...
        case 'DELETE':
            if (preg_match('/^\/cpf-id\/(\d+)\/?$/', $path, $matches)) {
                // DELETE /base-senha-cpf/cpf-id/{cpf_id}
                $_GET['cpf_id'] = $matches[1];
                $controller->deleteByCpfId();
            } elseif (preg_match('/^\/(\d+)\/?$/', $path, $matches)) {
                // DELETE /base-senha-cpf/{id}
                $_GET['id'] = $matches[1];
                $controller->delete();
            } else {
                http_response_code(404);
    ...
    error_log("ROUTE_BASE_SENHA_CPF: Erro fatal - " . $e->getMessage());
