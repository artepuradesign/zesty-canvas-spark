...
require_once __DIR__ . '/../src/controllers/BaseCnpjMeiController.php';
...
try {
    ...
    $controller = new BaseCnpjMeiController($db);
    $method = $_SERVER['REQUEST_METHOD'];
    $path = parse_url($_SERVER['REQUEST_URI'], PHP_URL_PATH);

    // Remover prefixo da API se presente
    $basePaths = ['/api/base-cnpj-mei', '/base-cnpj-mei'];
    ...
    switch ($method) {
        ...
        case 'DELETE':
            if (preg_match('/^\/cpf\/(\d+)\/?$/', $path, $matches)) {
                // DELETE /base-cnpj-mei/cpf/{cpf_id}
                $_GET['cpf_id'] = $matches[1];
                $controller->deleteByCpfId();
            } elseif (preg_match('/^\/(\d+)\/?$/', $path, $matches)) {
                // DELETE /base-cnpj-mei/{id}
                $_GET['id'] = $matches[1];
                $controller->delete();
            } else {
                http_response_code(404);
    ...
    error_log("ROUTE_BASE_CNPJ_MEI: Erro fatal - " . $e->getMessage());
