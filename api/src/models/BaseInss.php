<?php
// src/models/BaseInss.php

class BaseInss {
    private $db;
    private $table = 'base_inss';
    ...
    public function create($data) {
        $fields = [
            'cpf_id', 'cpf', 'nb', 'entidade', 'especie', 'especie_descricao', 'valor'
        ];
        
        $setFields = [];
        $values = [];
        
        // cpf_id é obrigatório
    ...
    public function update($id, $data) {
        $fields = [
            'nb', 'entidade', 'especie', 'especie_descricao', 'valor'
        ];
    ...
    public function deleteByCpf($cpf) {
        $query = "DELETE FROM {$this->table} WHERE cpf = ?";
        $stmt = $this->db->prepare($query);
