<?php
// src/controllers/LoginGmailController.php

require_once __DIR__ . '/../utils/Response.php';
require_once __DIR__ . '/../middleware/AuthMiddleware.php';
require_once __DIR__ . '/../models/LoginGmail.php';

class LoginGmailController {
    private $db;
    private $model;

    public function __construct($db) {
        $this->db = $db;
        $this->model = new LoginGmail($db);
    }

    /**
     * GET /login-gmail/logins - Listar logins disponíveis
     */
    ...
    /**
     * POST /login-gmail/comprar - Comprar login
     */
    public function comprar() {
        try {
            ...
            // Buscar preço do módulo 163
            $moduleQuery = "SELECT price FROM modules WHERE id = 163 LIMIT 1";
            ...
            error_log("LOGIN_GMAIL: Plano: {$planName}, Desconto: {$discountPercent}%, Original: R$ {$precoOriginal}, Final: R$ {$preco}");
            ...
            $transDesc = "Compra Login Gmail: {$login['email']}";
            ...
            $consultationQuery = "INSERT INTO consultations (user_id, module_type, document, cost, result_data, status, ip_address, user_agent, metadata, created_at, updated_at) 
                                 VALUES (?, 'login_gmail', ?, ?, NULL, 'completed', ?, ?, ?, NOW(), NOW())";
            ...
            $ccDesc = "Compra Login Gmail: {$login['email']}";
            $ccMeta = json_encode([
                'source' => 'login-gmail',
                'login_id' => $loginId,
                'compra_id' => $compraId,
            ...
            error_log("LOGIN_GMAIL: Compra realizada - User: {$userId}, Login: {$loginId}, Preço: {$preco}, Transaction: {$transactionId}");
            ...
            error_log("LOGIN_GMAIL ERRO: " . $e->getMessage());
            Response::error('Erro ao processar compra: ' . $e->getMessage(), 500);
        }
    }

    /**
     * GET /login-gmail/minhas-compras - Listar compras do usuário
     */
    ...
    /**
     * GET /login-gmail/provedores - Listar provedores ativos
     */
    ...
    /**
     * POST /login-gmail/criar - Criar novo login (admin/suporte)
     */
    public function criar() {
        try {
            ...
            $query = "INSERT INTO login_gmail (module_id, email, senha, provedor, cpf, saldo, pontos, status, observacao, ativo) 
                      VALUES (163, ?, ?, ?, ?, ?, ?, ?, ?, 1)";
            $stmt = $this->db->prepare($query);
            $stmt->execute([
                $input['email'],
                $input['senha'],
                $input['provedor'] ?? 'gmail',
                $input['cpf'] ?? null,
                $input['saldo'] ?? 0,
                $input['pontos'] ?? 0,
                $input['status'] ?? 'virgem',
    ...
    /**
     * PUT /login-gmail/atualizar - Atualizar login (admin/suporte)
     */
    public function atualizar() {
        try {
            ...
            $params[] = $id;
            $query = "UPDATE login_gmail SET " . implode(', ', $sets) . ", updated_at = NOW() WHERE id = ?";
            $stmt = $this->db->prepare($query);
            $stmt->execute($params);
    ...
    /**
     * DELETE /login-gmail/excluir - Excluir login (admin/suporte)
     */
    public function excluir() {
        try {
            ...
            // Soft delete
            $query = "UPDATE login_gmail SET ativo = 0, updated_at = NOW() WHERE id = ?";
            $stmt = $this->db->prepare($query);
            $stmt->execute([$id]);
