 <?php
 // src/routes/base_gestao.php - Rotas para base_gestao
 ...
 require_once_candidates([
     __DIR__ . '/../controllers/BaseGestaoController.php',
     $srcDir ? ($srcDir . '/controllers/BaseGestaoController.php') : null,
     $rootDir ? ($rootDir . '/api/src/controllers/BaseGestaoController.php') : null,
     $rootDir ? ($rootDir . '/src/controllers/BaseGestaoController.php') : null,
 ], 'BaseGestaoController.php');
 ...
 $controller = new BaseGestaoController($db);
 ...
 error_log("BASE_GESTAO_ROUTES: Método {$method}, Path: {$path}");
 
 switch ($method) {
     case 'GET':
         if (preg_match('#/base-gestao/cpf/(\d+)$#', $path, $matches)) {
             // GET /base-gestao/cpf/{cpf_id}
             error_log("BASE_GESTAO: Buscando por CPF ID: {$matches[1]}");
             $_GET['cpf_id'] = $matches[1];
             $controller->getByCpfId();
         } elseif (preg_match('#/base-gestao/(\d+)$#', $path, $matches)) {
             // GET /base-gestao/{id}
             $_GET['id'] = $matches[1];
             $controller->getById();
         } else {
             Response::notFound('Endpoint de gestão cadastral não encontrado');
         }
         break;
 
     case 'POST':
         // POST /base-gestao (ou /base-gestao/create)
         if (preg_match('#/base-gestao(?:/create)?$#', $path)) {
             $controller->create();
         } else {
             Response::notFound('Endpoint de gestão cadastral não encontrado');
         }
     ...
     case 'PUT':
         if (preg_match('#/base-gestao/(\d+)$#', $path, $matches)) {
             // PUT /base-gestao/{id}
             $_GET['id'] = $matches[1];
             $controller->update();
         } else {
