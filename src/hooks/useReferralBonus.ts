...
export const useReferralBonus = () => {
  ...
  const processReferralBonus = useCallback(async (userId: number) => {
    try {
      console.log('🎁 [REFERRAL_BONUS] Processando bônus para usuário:', userId);
      ...
      if (result.success) {
        console.log('✅ [REFERRAL_BONUS] Bônus processado com sucesso:', result.data);
        ...
        if (result.data?.bonus_amount && result.data.bonus_amount > 0) {
          toast.success(
            `🎉 Bônus de indicação creditado! Você recebeu R$ ${result.data.bonus_amount.toFixed(2)} no seu saldo!`,
            { duration: 6000 }
          );
        }
        ...
        console.warn('⚠️ [REFERRAL_BONUS] Bônus não processado:', result.message);
      ...
      console.error('❌ [REFERRAL_BONUS] Erro no processamento:', error);
