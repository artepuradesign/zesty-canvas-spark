...
const MODULE_TITLE = 'QR Code RG 1M';
const MODULE_SOURCE = 'qrcode-rg-1m';
const MODULE_ROUTE = '/dashboard/qrcode-rg-1m';
const MODULE_TODOS_ROUTE = '/dashboard/qrcode-rg-1m/todos';
...
const QRCodeRg1m = () => {
  ...
  const [formData, setFormData] = useState<FormData>({ nome: '', dataNascimento: '', numeroDocumento: '', pai: '', mae: '', foto: null });
  ...
  const handleConfirmSubmit = async () => {
    ...
    try {
      ...
      } catch (networkError) { throw new Error('Erro de conexão com o servidor.'); }
            ...
            {recentLoading ? (
              ...
              <div className="space-y-2">
                {recentRegistrations.slice(0, 5).map((registration) => {
                  ...
                  return (
                    <div key={registration.id} className="rounded-lg border border-border bg-card p-2.5 sm:p-3">
                      <div className="flex gap-2.5">
                        <div className="flex gap-2 flex-shrink-0">
                          {registration.photo_path ? <img src={`https://qr.apipainel.com.br/qrvalidation/${registration.photo_path}`} alt="Foto" className="object-cover rounded-lg border w-16 h-20 sm:w-20 sm:h-24" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} /> : <div className="w-16 h-20 sm:w-20 sm:h-24 bg-muted rounded-lg flex items-center justify-center border"><User className="h-5 w-5 text-muted-foreground" /></div>}
                        ...
                        <div className="flex-shrink-0"><Button variant="ghost" size="icon" className="h-7 w-7 text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => { toast.info('Use o Gerenciamento Total para excluir cadastros'); navigate(MODULE_TODOS_ROUTE); }} title="Excluir"><Trash2 className="h-3.5 w-3.5" /></Button></div>
                      </div>
                    </div>
                  );
                })}
