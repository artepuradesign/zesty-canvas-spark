...
const ConsultaPersonalizada = () => {
  ...
  const calculateTotalPrice = () => {
    ...
    return calculateDiscountedPrice(basePrice, userPlan);
  };
  ...
  const handleConsulta = async () => {
    ...
    try {
      ...
      toast.success(
        `Consulta personalizada realizada! Valor descontado: R$ ${finalPrice.toFixed(2)}`,
        {
          style: {
  ...
  const { finalPrice, discount } = calculateTotalPrice();
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="text-xs text-gray-600 dark:text-gray-400 border-t pt-2">
                  Saldo atual: {formatCurrency(userBalance)}
                </div>
              </div>

              <Button
                onClick={handleConsulta}
                disabled={loading || !cpf || cpf.length !== 11 || selectedOptions.length === 0 || userBalance < finalPrice}
