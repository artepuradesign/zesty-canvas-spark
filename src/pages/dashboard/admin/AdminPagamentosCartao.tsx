...
import { CreditCard, TrendingUp, Users, Calendar, Search, Filter, RefreshCw } from 'lucide-react';
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <DashboardTitleCard
        ...
        right={
          <Button onClick={() => loadTransactions(200, 'card')} disabled={isLoading} variant="outline" size="sm" className="h-8 sm:h-9">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <Card>
          ...
          <CardContent className="p-3 pt-0 sm:p-6 sm:pt-0">
            <div className="text-lg sm:text-2xl font-bold">{formatBrazilianCurrency(stats?.payment_card || cardStats.totalValue)}</div>
      ...
      <Card>
        ...
        <CardContent className="p-3 sm:p-6 pt-0 sm:pt-0">
          {isLoading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">Nenhuma transação de cartão encontrada</div>
          ) : (
            <div className="space-y-3">
              {filteredTransactions.map((transaction) => (
                <div key={transaction.id} className="border rounded-lg p-3 sm:p-4 space-y-2 bg-card border-l-4 border-l-blue-500">
