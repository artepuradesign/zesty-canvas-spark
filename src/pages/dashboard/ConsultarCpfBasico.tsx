  ...
  try {
    ...
    if (searchResult.success && searchResult.data) {
      ...
      try {
        ...
        const moduleTypeTitle = (metadata?.module_title || metadata?.moduleTypeTitle || 'CPF BÁSICO').toString();
        ...
        const registroPayload = {
          ...
          cost: finalCost, // VALOR COM DESCONTO JÁ APLICADO (preço do módulo ID 21 com desconto)
          status: 'completed',
          ...
          saldo_usado: saldoUsado, // Incluir o tipo de saldo usado
            metadata: {
              source: 'consultar-cpf-basico',
              page_route: window.location.pathname,
              module_title: moduleTypeTitle,
              discount: metadata.discount || 0,
              original_price: metadata.original_price || finalCost, // preço original sem desconto do módulo ID 21
              ...
              module_id: 21, // ID do módulo CPF BÁSICO
      ...
      if (preCheck.success && preCheck.exists && preCheck.data) {
        ...
        const registroPayload = {
          ...
          saldo_usado: saldoUsado,
          metadata: {
            source: 'consultar-cpf-basico-precheck',
            page_route: window.location.pathname,
            module_title: moduleTypeTitle,
            discount: metadata.discount || 0,
        ...
        if (atitoResult.success) {
          ...
          if (finalCheck.success && finalCheck.exists && finalCheck.data) {
            ...
            const registroPayload = {
              ...
              saldo_usado: saldoUsado,
              metadata: {
            source: 'consultar-cpf-basico-railway-flow',
                page_route: window.location.pathname,
                module_title: moduleTypeTitle,
                discount: metadata.discount || 0,
                original_price: metadata.original_price || finalCost,
...
const ConsultarCpfPuxaTudo = () => {
    ...
    try {
      ...
      if (response.success && response.data && Array.isArray(response.data)) {
        const getModuleLabel = (route?: string) => {
          ...
          if (!r) return '-';
          if (r.includes('/dashboard/consultar-cpf-basico')) return 'CPF BÁSICO';
  ...
  const performSearch = async () => {
    ...
    try {
      // Usar preço do módulo ID 21 da API - aguardar carregamento se necessário
      ...
      console.log('💰 Preço original do módulo ID 21:', originalPrice);
      ...
      const baseCpfResult = await consultarCPFComRegistro(cpf, finalPrice, {
        ...
        module_title: currentModule?.title || 'CPF BÁSICO'
      });
...
Todos os direitos reservados.`;
  ...
  // Calcular preço com desconto para exibição usando módulo ID 21
