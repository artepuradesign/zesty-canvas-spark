...
const ActivityList: React.FC<ActivityListProps> = ({ activities }) => {
  const getActivityIcon = (type: string) => {
    switch (type) {
      ...
      case 'consultation': return <Search className="h-4 w-4 text-orange-500" />;
      case 'recharge': return <CreditCard className="h-4 w-4 text-purple-500" />;
      case 'user_online': return <Users className="h-4 w-4 text-green-500" />;
  ...
  const getActivityColor = (type: string) => {
    switch (type) {
      ...
      case 'consultation': return 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800';
      case 'recharge': return 'bg-purple-50 dark:bg-purple-900/20 border-purple-200 dark:border-purple-800';
      case 'user_online': return 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800';
