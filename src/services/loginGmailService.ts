...
export interface LoginGmailItem {
  id: number;
  module_id: number;
  email: string;
  senha: string;
...
export interface LoginGmailCompra {
  id: number;
  module_id: number;
  user_id: number;
  login_id: number;
...
export const loginGmailService = {
  async listLogins(params: { limit?: number; offset?: number; search?: string } = {}) {
    ...
    const endpoint = `/login-gmail/logins${qs.toString() ? `?${qs.toString()}` : ''}`;
    return apiRequest<{ data: LoginGmailItem[]; pagination: { total: number; limit: number; offset: number } }>(endpoint);
  },
  ...
  async minhasCompras(params: { limit?: number; offset?: number } = {}) {
    ...
    const endpoint = `/login-gmail/minhas-compras${qs.toString() ? `?${qs.toString()}` : ''}`;
    return apiRequest<{ data: LoginGmailCompra[]; pagination: { total: number; limit: number; offset: number } }>(endpoint);
  },
  ...
  async criar(data: { email: string; senha: string; provedor?: string; cpf?: string; saldo?: number; pontos?: number; status?: string; observacao?: string }) {
    return apiRequest<{ id: number }>('/login-gmail/criar', {
      method: 'POST',
  ...
  async listProvedores() {
    return apiRequest<LoginProvedor[]>('/login-gmail/provedores');
  },

  async atualizar(data: { id: number; [key: string]: any }) {
    return apiRequest<{ id: number }>('/login-gmail/atualizar', {
      method: 'PUT',
      body: JSON.stringify(data),
    });
  },
  ...
  async excluir(id: number) {
    return apiRequest<{ id: number }>('/login-gmail/excluir', {
      method: 'DELETE',
